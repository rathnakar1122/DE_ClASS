...
def branch(ti, **kwargs):
    day_index = ti.xcom_pull(task_ids='weekday', key='day')
    return 'task_for_' + tabDays[day_index]
...
for day in range(0, 7):  # Include all days
    task = DummyOperator(
        task_id='task_for_' + tabDays[day],
        dag=dag
    )
    fork >> task
